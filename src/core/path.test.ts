...
describe('buildPath', () => {
  ...
  /**
   * ★ 기본 규칙(`DEFAULT_CONFIG`)이 만드는 경로 — **갤러리 앨범 규칙이기도 하다.**
   *
   * 전달이 «갤러리 다중선택 → 카톡» 이라, 폴더 맨 끝 이름이 곧 앨범명이다.
   * 동/호수를 폴더가 아니라 **파일명 맨 앞**에 두어 앨범을 단지 수만큼으로 줄이고 이름도 유일하게 한다
   * (2026-08-03 사용자 결정. 예전 `단지/동호수` 2단은 단지가 달라도 앨범 「01」이 겹쳤다).
   */
  it('★ 기본 규칙 — 폴더는 단지까지, 동/호수는 파일명 맨 앞', () => {
    const 기본: PathConfig = { ...cfg, folderKeys: ['danji'], fileKeys: ['ho', 'work', 'phase'] }
    const f: Fields = {
      danji: '행복아파트',
      ho: '101동 1502호',
      work: '타일 보수',
    ...
    expect(buildPath(f, 기본, 1)).toBe('행복아파트/101동 1502호_타일 보수_작업 후_001.jpg')
    // 단지가 달라도 폴더(=앨범)는 단지명이라 안 겹친다
    expect(buildPath({ ...f, danji: '중리' }, 기본, 2)).toBe(
      '중리/101동 1502호_타일 보수_작업 후_002.jpg',
    )
  })
